...
/*   Updated: 2023/04/10 14:30:27 by hdelmas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main()
{
	...
	map = (char **)malloc(sizeof(char *) * (X_RES + 100) / DIV);
	i = -1;
	while(++i < (X_RES + 100) / DIV)
	{
		sub = (char *)malloc(sizeof(char) * (((X_RES + 100) / DIV) + 1));
		sub[((X_RES + 100) / DIV)] = '\0';
		j = -1;
		while (++j < (X_RES + 100) / DIV)
		{
			val = '0';
			if (i == 0 || j == 0 || i == ((X_RES + 100) / DIV) - 1|| j == ((X_RES + 100) / DIV) - 1 /*|| i == 400 || j == 400 || i == 600 || j == 600*/)
				val = '1';
			sub[j] = val;
		}
		map[i] = sub;
